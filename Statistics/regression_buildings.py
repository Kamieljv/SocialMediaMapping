...
pairs = np.array([[id, bd, twt] for id, bd, twt in zip(ids, bd_cnt, twt_dens)])
pairs_nozero = np.array([[id, bd, twt] for id, bd, twt in zip(ids, bd_cnt, twt_dens) if twt > 0.001])
...
#ax.scatter(pairs_spear_h[:,2], pairs_spear_h[:,1], c='red', marker='.', label='Highly sign. data')
legend = ax.legend(loc='upper right', shadow=False)
plt.xlabel('Tweet density ($twt/km^2$)')
plt.ylabel('Nr. of Buildings')
plt.title('Different fits for Building Count vs. Tweet density')
plt.show()
...
with open(root_dir+outfile+'.csv', 'w', newline='') as file:
    ...
    for i, id in enumerate(ids):
        csvwriter.writerow([id, round(pred_raw[i],4), round(pred_nozero[i],4), round(pred_spear_m[i],4), round(pred_spear_h[i],4),
                            round(pred_raw[i]-bd_cnt[i],4), round(pred_nozero[i]-bd_cnt[i],4), round(pred_spear_m[i]-bd_cnt[i],4), round(pred_spear_h[i]-bd_cnt[i],4)])
print('=> File written.')
